int fadeValue;
int const ledGreenPin = 9;
int const ledBluePin = 10;
int const ledRedPin = 11;
int const buttonGreenPin = 2;
...
for(fadeValue=0; fadeValue<=255; fadeValue++) {
  ...
  analogWrite(ledBluePin, fadeValue);
  delay(30);
}
 analogWrite(ledBluePin, 0);
for(fadeValue=0; fadeValue<=255; fadeValue++) {
...
void crossFade(int color[3]) {
  ...
  for (int i = 0; i <= 1020; i++) {
    ...
    analogWrite(ledRedPin, redVal);   // Write current values to LED pins
    analogWrite(ledGreenPin, grnVal);      
    analogWrite(ledBluePin, bluVal); 
